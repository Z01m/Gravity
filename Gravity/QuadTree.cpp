...
QuadTree::QuadTree(const BoundingBox2D& boundingBox)
...
void QuadTree::ApplyGForcesToEntity(Circ* entity)
{
    m_tree->ApplyGForceToEntity(entity, BarnesHut);
}

void QuadTree::Build(const std::vector<Circ*>& entities)
{
    for (Circ* entity : entities)
    {
        m_tree->Push(entity);
    }
}
